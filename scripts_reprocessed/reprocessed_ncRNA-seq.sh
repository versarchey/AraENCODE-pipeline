####################
#in the process of miRNA analysis, firstly filter the adapter with cutadapt, then mapped reads to the genome with mirdeep2, and normalized  the expression of miRNA annotated in miRBase. siRNA analysis first filters out tRNA, rRNA and miRNA in Arabidopsis genome, and then found 21nt and 24nt siRNA reads by script.
#####################
...
################################ mapped reads to tair10 by mirdeep2
...
###### Mapped unmapped reads to tair10 genome and filter 21nt 24nt siRNA
bowtie $genomeindex -q -a -v 0 --al ${i%%_*}_map --un ${i%%_*}_nomap ./${i%%.*}_un -S ${i%%_*}.tair10.sam
